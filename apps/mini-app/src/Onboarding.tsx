...
export function OnboardingView({ onComplete, lang }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      await api("/api/v2/onboarding", { method: "POST", idempotencyKey: crypto.randomUUID(), body: typedBody<"completeOnboarding">({ sex, age: Number(age), heightCm: Number(heightCm), weightKg: Number(weightKg), goal, level, equipment, dietPrefs, trainingWeekdays: days, limitations: limitations.trim() || "none" }) });
      onComplete();
    } catch (err) {
