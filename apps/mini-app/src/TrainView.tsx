...
export function TrainView({ lang }: { lang: Lang }) {
  ...
  const openSwap = async (index: number) => { setSwapFor(index); setActionBusy(`swap:${index}`); try { const data = await api<{ alternatives: Array<{ id: string; name: string }> }>(`/api/v2/workout/swap?index=${index}`); setSwapChoices(data.alternatives); } catch (err) { setError(err); } finally { setActionBusy(null); } };
  ...
  const addCustom = async () => { const name = customName.trim(); if (name.length < 2) return; setActionBusy("custom"); try { const result = await api<{ name: string; videoUrl?: string; videoTitle?: string }>("/api/v2/workout/custom", { method: "POST", idempotencyKey: crypto.randomUUID(), body: jsonBody({ name }) }); setWorkout((current) => { if (!current) return current; const exercise: LoggerExercise = { index: current.exercises.length, name: result.name, metric: "reps", sets: 1, ...(result.videoUrl ? { videoUrl: result.videoUrl, videoTitle: result.videoTitle } : {}) }; const next = { ...current, exercises: [...current.exercises, exercise] }; persistDraft(next, logDate, copiedFrom); return next; }); setCustomName(""); setShowCustom(false); } catch (err) { setError(err); } finally { setActionBusy(null); } };
  ...
  const save = async () => { if (!workout) return; setSaving(true); setSaved(false); const targetDate = logDate; try { await api("/api/v2/workout/save", { method: "POST", idempotencyKey: crypto.randomUUID(), body: jsonBody({ entries: workout.exercises.filter((e) => e.setsDone?.some((set) => set.reps || set.seconds || set.meters)).map((e) => ({ name: e.name, sets: e.setsDone, ...(e.rpe !== undefined ? { rpe: e.rpe } : {}) })), ...(targetDate ? { date: targetDate } : {}) }) }); localStorage.removeItem(draftKey); setDrafted(false); setSaved(true); setSaveNoteDate(targetDate); window.Telegram?.WebApp.HapticFeedback?.notificationOccurred("success"); if (targetDate) { setHistory(null); load(); } } catch (err) { setError(err); } finally { setSaving(false); } };

  if (loading) return <Loading />;
  if (error) return <ErrorState lang={lang} error={error} retry={load} />;
  const tabs = <div className="button-row"><button className={subview === "today" ? "button button-primary" : "button button-ghost"} onClick={() => setSubview("today")}>{t(lang, "train_tab_today")}</button><button className={subview === "history" ? "button button-primary" : "button button-ghost"} onClick={() => setSubview("history")}>{t(lang, "train_tab_history")}</button></div>;
