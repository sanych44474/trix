...
import {
  clampRest, density, DEFAULT_REST_PREFS, DEFAULT_REST_SEC, EMPTY_QUALITY, fmtDuration, fmtRest,
...
// The session clock (when it started + the rest tally) used to live only in component state, so
// switching tabs or re-opening the app mid-workout reset it: a two-hour session came out as
// "2:44" -- the time since the first rest after coming back. It is now kept per date.
const SESSION_CLOCK_KEY = "trix:v2:session-clock";
interface SessionClock { date: string; startedAt: number; quality: SessionQuality }

function loadSessionClock(date: string): SessionClock | null {
  try {
    const raw = localStorage.getItem(SESSION_CLOCK_KEY);
    const clock = raw ? JSON.parse(raw) as SessionClock : null;
    return clock && clock.date === date && typeof clock.startedAt === "number" ? { ...clock, quality: { ...EMPTY_QUALITY, ...clock.quality } } : null;
  } catch { return null; }
}

function saveSessionClock(clock: SessionClock | null): void {
  try { if (clock) localStorage.setItem(SESSION_CLOCK_KEY, JSON.stringify(clock)); else localStorage.removeItem(SESSION_CLOCK_KEY); } catch { /* storage is optional */ }
}

// Below this, "% of time working" is noise (one set and a rest reads as 18%), so it isn't shown.
const DENSITY_MIN_SESSION_SEC = 10 * 60;
// A clock older than this is a leftover (saved this morning, edited tonight), not a session length.
...
export function TrainView({ lang, gamification }: { lang: Lang; gamification?: Dashboard["gamification"] }) {
  ...
  // Any edit after a save means there are unsaved changes again, so the "saved" state goes too.
  ...
  useEffect(() => {
    if (!workout || logDate || sessionStartedAt == null) return;
    saveSessionClock({ date: workout.date, startedAt: sessionStartedAt, quality });
  }, [workout?.date, logDate, sessionStartedAt, quality]);

  const load = () => {
    setLoading(true); setError(null);
    api<WorkoutToday>("/api/v2/workout/today").then((raw) => {
      const data = hydrateSaved(raw);
      // Re-opening a day that is already logged says so, instead of looking like a blank form.
      setLogDate(null); setCopiedFrom(null); setSaved(Boolean(raw.saved?.length)); setSaveNoteDate(null);
      const clock = loadSessionClock(data.date);
      if (clock) { setSessionStartedAt(clock.startedAt); setQuality(clock.quality); }
      try {
        const stored = localStorage.getItem(draftKey);
        const draft = stored ? JSON.parse(stored) as WorkoutToday & { logDate?: string; copiedFrom?: string; savedAt?: number } : null;
        if (draft?.date === data.date && Array.isArray(draft.exercises)) {
          // A draft carrying savedAt is the exact list that was last saved (kept on purpose -- see
          // save()), so it restores swaps/custom exercises/order that the server log can't.
          setWorkout({ ...data, exercises: draft.exercises }); setLogDate(draft.logDate ?? null); setCopiedFrom(draft.copiedFrom ?? null);
          setDrafted(!draft.savedAt); if (draft.savedAt) setSaved(true); return;
        }
  ...
  const updateSet = (index: number, setIndex: number, key: "weight" | "reps" | "seconds" | "meters" | "rpe", value: number) => { setSessionStartedAt((current) => current ?? Date.now()); setWorkout((current) => {
  ...
  // The save button sits at the bottom of a long list, but the "saved" summary (and any error)
  // renders at the top -- out of sight, so a save looked like it did nothing and got tapped again.
  const scrollToTop = () => { try { window.scrollTo({ top: 0, behavior: "smooth" }); } catch { window.scrollTo(0, 0); } };
  const save = async () => { if (!workout) return; setSaving(true); setSaved(false); setActionError(null); const targetDate = logDate; try { const entries = workout.exercises.flatMap((e) => e.setsDone && e.setsDone.some((set) => set.reps || set.seconds || set.meters) ? [{ name: e.name, sets: e.setsDone, ...(e.rpe !== undefined ? { rpe: e.rpe } : {}) }] : []); const result = await api<SaveResponse>("/api/v2/workout/save", { method: "POST", idempotencyKey: crypto.randomUUID(), body: typedBody<"saveWorkout">({ entries, ...(targetDate ? { date: targetDate } : {}) }) }); closeOpenRest(); try { if (targetDate) localStorage.removeItem(draftKey); else localStorage.setItem(draftKey, JSON.stringify({ ...workout, logDate: null, copiedFrom, savedAt: Date.now() })); } catch { /* storage is optional */ } setDrafted(false); setSaved(true); setSaveNoteDate(targetDate); const elapsedSec = sessionStartedAt == null ? 0 : Math.round((Date.now() - sessionStartedAt) / 1000); const clockValid = elapsedSec > 0 && elapsedSec <= MAX_SESSION_SEC; setSummary({ prExercises: result.prExercises ?? [], newBadges: result.newBadges ?? [], level: result.level ?? 1, leveledUp: result.leveledUp === true, totalWorkouts: result.totalWorkouts ?? 0, sets: entries.reduce((total, e) => total + e.sets.filter((set) => set.reps || set.seconds || set.meters).length, 0), elapsedSec: clockValid ? elapsedSec : 0, restTotalSec: quality.restTotalSec, densityPct: clockValid && elapsedSec >= DENSITY_MIN_SESSION_SEC ? density(sessionStartedAt, quality.restTotalSec, Date.now()) : null, bestStreak: quality.bestStreak }); window.Telegram?.WebApp.HapticFeedback?.notificationOccurred("success"); scrollToTop(); if (targetDate) { setHistory(null); load(); } } catch (err) {
    ...
    setActionError(err); scrollToTop();
  ...
  return <div className="view-stack"><div className="eyebrow">{t(lang, "guided_logger_eyebrow", { date: logDate ?? workout.date })}</div><div className="page-title"><h1>{t(lang, "training_session_title")}</h1><span>{filled}/{workout.exercises.length}</span></div>{tabs}{gamification && !saved && <div className="train-progress"><span className="tag">{t(lang, "level_n", { n: gamification.level })}</span><div className="progress-track"><span style={{ width: `${gamification.needed ? Math.min(100, gamification.intoLevel / gamification.needed * 100) : 100}%` }} /></div>{gamification.streak ? <span className="tag">{t(lang, "streak_weeks", { n: gamification.streak })}</span> : null}</div>}{actionError !== null && <Card tone="muted"><div className="error-state"><strong>{t(lang, "generic_error")}</strong><button className="button button-ghost" onClick={() => setActionError(null)}>{t(lang, "close")}</button></div></Card>}{(logDate || copiedFrom) &&<div className="button-row"><button className="text-button" onClick={backToToday}>{t(lang, "back_to_today")}</button></div>}{saved && summary ? <Card tone="accent">
      ...
      <div className="session-stats">
        ...
        {summary.elapsedSec > 0 && <div><strong>{fmtDuration(summary.elapsedSec)}</strong><small>{t(lang, "summary_elapsed")}</small></div>}
